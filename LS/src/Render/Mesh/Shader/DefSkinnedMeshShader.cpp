...
{
	_vpUniform = _shader.getUniform("vp");
	_vUniform = _shader.getUniform("v");
...
void DefSkinnedMeshShader::assignUniforms(RenderInfo &fD, const ModelPart& part, const glm::mat4 &modelMatrix, MaterialLink *matLink) const {
	...
	//Send matrixes
	glUniformMatrix4fv(_vpUniform, 1, GL_FALSE, (const GLfloat*)&(fD._VP));
	glUniformMatrix4fv(_vUniform, 1, GL_FALSE, (const GLfloat*)&(fD._V));
